...
class MaskRCNNMaskHead(head.Head):
  ...
  def predict(self, features, num_predictions_per_location=1):
    ...
    # CURRENTLY WE IMPLEMENT ONLY THE 2 POSSIBILITIES AS SHOWN IN MRCNN PAPER
    with slim.arg_scope(self._conv_hyperparams_fn()):
      if self._backbone is None or self._backbone == 'C4':
        ...
        with tf.name_scope('MaskBase'):
          mask_features = features
          for _ in range(self._mask_prediction_num_conv_layers):
            mask_features = slim.conv2d(mask_features,
                                        num_outputs=num_conv_channels,
                                        activation_fn=tf.nn.relu,
                                        kernel_size=[3, 3])
          mask_features = slim.conv2d_transpose(
              mask_features,
              num_outputs=num_conv_channels,
              activation_fn=tf.nn.relu,
              kernel_size=[2, 2],
              stride=2)
        with tf.name_scope('MaskPredictor'):
          num_masks = 1 if self._masks_are_class_agnostic else self._num_classes
          mask_predictions = slim.conv2d(mask_features,
                                         num_outputs=num_masks,
                                         activation_fn=None,
                                         kernel_size=[3, 3])
          return tf.expand_dims(
              tf.transpose(mask_predictions, perm=[0, 3, 1, 2]), axis=1)
      else:
