...
class MaskRCNNBoxPredictor(box_predictor.BoxPredictor):
  ...
  def __init__(self,
    ...
    self._box_class_num_conv_layers = box_class_num_conv_layers
  ...
  def _predict(self,
    ...
    if prediction_stage == 2:
      if self._backbone is None or self._backbone == 'C4':
        tf.logging.info("`backbone` is not specified. Using the default C4 "
                        "backbone.")
      elif self._backbone == 'FPN':
        flattened_image_features = slim.flatten(image_feature)
        # TODO: In Matterport implementation, BN is used before relu.
        # TODO: In Matterport implementation, they are using Conv2D with
        # (pool_size^2) for the first FC.
        with slim.arg_scope(self._fc_hyperparams_fn()):
          flattened_image_features = slim.fully_connected(
              flattened_image_features,
              self._box_class_num_conv_layers,
              activation_fn=tf.nn.relu,
              scope='BoxEncodingClassBaseFC1')
          image_feature = slim.fully_connected(
              flattened_image_features,
              self._box_class_num_conv_layers,
              activation_fn=tf.nn.relu,
              scope='BoxEncodingClassBaseFC2')
      else:
        raise ValueError('Backbone specified is not supported.')
