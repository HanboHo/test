...
class FasterRCNNFeatureExtractor(object):
  ...
  def restore_from_classification_checkpoint_fn(self, **kwargs):
    ...
    Returns:
      ...
      the model graph.
    """
    return lib.restore_from_classification_checkpoint_fn(**kwargs)
...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def predict(self, preprocessed_inputs, true_image_shapes, **params):
    ...
    # (1.1.) Decoder 1                                                         #
    # ======================================================================== #
    with tf.variable_scope(self.first_stage_feature_extractor_scope):
    ...
    # ======================================================================== #
    with tf.variable_scope(self.first_stage_feature_extractor_scope):
        with tf.variable_scope("Decoder2", reuse=False):
            image_features_btm_up = list(
                reversed(list(top_down_feature.items())))
            top_down_feature_2 \
                = feature_map_generators.top_down_feature_maps(
                    image_features_btm_up, residual_depth=256)
    ...
    with tf.variable_scope('SemanticPredictor'):
        prediction_dict['semantic_predictions'] = \
            self._predict_semantic_labels(top_down_feature['decoder_M2'],
    ...
    with tf.variable_scope('InstancePredictor'):
        ...
        prediction_dict['instance_predictions'] = \
            self._predict_semantic_labels(top_down_feature_2['decoder_M2'],
                                          num_of_instances)
  ...
  def predict_original(self, preprocessed_inputs, true_image_shapes, **params):
    """Predicts unpostprocessed tensors from input tensor.
  ...
  def _predict_semantic_labels(self, features, num_labels):
    ...
    with slim.arg_scope(self._first_stage_semantic_arg_scope_fn()):
        ...
        decoder_features = slim.repeat(
            ...
            self._first_stage_semantic_kernel_size,
            scope='decoder_conv')
        # with tf.variable_scope('decoder_conv'):
        ...
        return tf.cast(semantic_logits, dtype=tf.float32)
  ...
  def postprocess_original(self, prediction_dict, true_image_shapes, **params):
    """Convert prediction tensors to final detections.
    ...
    Raises:
      ValueError: If `predict` is called before `preprocess`.
    """
    with tf.variable_scope('FirstStagePostprocessor'):
    ...
    with tf.variable_scope('SecondStagePostprocessor'):
    ...
    with tf.variable_scope('SecondStagePostprocessorInferenceWithMask'):
      if self._number_of_stages == 3:
  ...
  def _postprocess_rpn(self,
    ...
    if self._mask_rcnn_box_predictor.get_prediction_head_backbone() is None \
        ...
        if self._is_training:
            ...
            with tf.variable_scope("Hard_Miner"):
                if not self._hard_example_miner:
  ...
  def _postprocess_logits(logits, true_image_shapes):
    ...
    prediction_logits = tf.map_fn(
        resize_logits, elems=[logits, true_image_shapes], dtype=logits.dtype)
    prediction_softmax = tf.nn.softmax(
        prediction_logits, axis=-1)
    prediction_probability = tf.reduce_max(
      prediction_softmax, axis=3, keepdims=True)
    prediction = tf.argmax(
      prediction_softmax, axis=3, output_type=tf.int32)
  ...
  def _loss_box_classifier(self,
    ...
    with tf.variable_scope('BoxClassifierLoss'):
  ...
  def _loss_kl_divergence(self,
    ...
    """
    with tf.variable_scope('KLDivergenceLoss'):
  ...
  def _add_summary_prediction_image(self, params):
    ...
    with tf.variable_scope('BoundingBoxes'):
