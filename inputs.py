...
def transform_input_data(tensor_dict,
  ...
  # Apply model preprocessing ops and resize input images and instance masks.
  original_image = tensor_dict[fields.InputDataFields.image]
  tensor_dict[fields.InputDataFields.original_image] = original_image
  tensor_dict[fields.InputDataFields.original_image_shape] = tf.shape(
      original_image)
  image = tensor_dict[fields.InputDataFields.image]
  preprocessed_resized_image, true_image_shape = model_preprocess_fn(
      tf.expand_dims(image, axis=0))
  # It is better to do this later during feature extraction because
  # preprocessing can cause the image to be recast into fp32
  # if use_bfloat16:
  ...
  if fields.InputDataFields.groundtruth_semantic in tensor_dict:
      segmentation_name = fields.InputDataFields.groundtruth_semantic
      # Images are resized and channel meaned so we cannot use it for
      # semantic labels.
      # preprocessed_resized_segmentation, _ = model_preprocess_fn(
      #     tf.expand_dims(tensor_dict[segmentation_name], axis=0))
      preprocessed_resized_segmentation = tf.image.resize_nearest_neighbor(
          tf.expand_dims(tensor_dict[segmentation_name], axis=0),
          tf.shape(preprocessed_resized_image)[1:3], align_corners=True)
      tensor_dict[segmentation_name] = tf.squeeze(
          preprocessed_resized_segmentation, axis=0)
  ...
  tensor_dict[fields.InputDataFields.groundtruth_classes] = tf.one_hot(
      ...
      dtype=zero_indexed_groundtruth_classes.dtype)
...
def pad_input_data_to_static_shapes(tensor_dict, max_num_boxes, num_classes,
  ...
  padding_shapes = {
      ...
      fields.InputDataFields.groundtruth_semantic: [height, width, 3],
  ...
  if fields.InputDataFields.original_image in tensor_dict:
    ...
    #     1024, 1024, 3 + num_additional_channels
    # ]
    padding_shapes[fields.InputDataFields.original_image] = [
        height, width, 3 + num_additional_channels
    ]
  if fields.InputDataFields.original_image_shape in tensor_dict:
    padding_shapes[fields.InputDataFields.original_image_shape] = [3]
...
def augment_input_data(tensor_dict, data_augmentation_options):
  ...
  """
  image_name = fields.InputDataFields.image
  segmentation_name = fields.InputDataFields.groundtruth_semantic

  tensor_dict[image_name] = tf.expand_dims(
      tf.cast(tensor_dict[image_name], dtype=tf.float32), 0)
  if fields.InputDataFields.groundtruth_semantic in tensor_dict:
    tensor_dict[segmentation_name] = tf.expand_dims(
        tensor_dict[segmentation_name], 0)
  ...
  tensor_dict[image_name] = tf.squeeze(tensor_dict[image_name], axis=0)
  if fields.InputDataFields.groundtruth_semantic in tensor_dict:
    tensor_dict[segmentation_name] = tf.squeeze(
        tensor_dict[segmentation_name], axis=0)
...
def _get_features_dict(input_dict):
  ...
  1. features dict is the dict that will be fed into the model as input.
  2. input_dict is the dict that contains the decoded dict from tfrecords and
     also preprocessed data.
  """
  hash_from_source_id = tf.string_to_hash_bucket_fast(
      input_dict[fields.InputDataFields.source_id], HASH_BINS)
  # TODO: We only change the input once the preprocessing completes. During
  # when preprocessing, it is possible that the data becomes fp32 due to
  # tf.image
  # MixPrecision for inputs
  features = {
      fields.InputDataFields.image:
          tf.cast(input_dict[fields.InputDataFields.image], dtype=fp_dtype),
  ...
  if fields.InputDataFields.filename in input_dict:
    features[fields.InputDataFields.filename] = input_dict[
        fields.InputDataFields.filename]
  if fields.InputDataFields.source_id in input_dict:
    features[fields.InputDataFields.source_id] = input_dict[
        fields.InputDataFields.source_id]
  return features


def create_train_input_fn(train_config, train_input_config,
                          model_config, dataset_name=None):
  ...
  Args:
    ...
    dataset_name: Name of dataset. Used for visualization and label mapping.
...
def create_eval_input_fn(eval_config, eval_input_config,
  ...
  def _eval_input_fn(params=None):
    ...
    dataset = INPUT_BUILDER_UTIL_MAP['dataset_build'](
        eval_input_config,
        batch_size=params['batch_size'] if params else
        eval_config.batch_size, # TODO we need to test this.
...
def create_predict_input_fn(model_config, predict_input_config,
  ...
  def _predict_input_fn(params=None):
    ...
    original_image = tf.expand_dims(
        input_dict[fields.InputDataFields.original_image], axis=0)
