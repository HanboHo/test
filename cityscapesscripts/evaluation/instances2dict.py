...
def instances2dict(imgList, imgNameList, verbose=False):
    imgCount     = 0
    instanceDict = {}
    ...
    if verbose:
        print("Processing {} images...".format(len(imgList)))

    for imgNp, imgName in zip(imgList, imgNameList):
        # Initialize label categories
        instances = {}
        for label in labels:
            instances[label.name] = []
        ...
        instanceDict[imgName] = instances
        imgCount += 1
