...
sys.path.append( os.path.normpath( os.path.join( os.path.dirname( __file__ ) , '..' , 'evaluation/cityscapesscripts/evaluation' ) ) )
...
CSUPPORT = True
from cityscapesscripts.evaluation.cityscapesscripts.evaluation import addToConfusionMatrix
...
def evaluateImgLists(predictionImgNameList, groundTruthImgNameList,
    ...
    for i in range(len(predictionImgList)):
        if instanceImgList is not None:
            instanceImg = instanceImgList[i]
        else:
            instanceImg = None
        predictionImgName = predictionImgNameList[i]
        groundTruthImgName = groundTruthImgNameList[i]
        predictionImg = predictionImgList[i]
        groundTruthImg = groundTruthImgList[i]
        #print "Evaluate ", predictionImgFileName, "<>", groundTruthImgFileName
        nbPixels += evaluatePair(
            predictionImgName, groundTruthImgName,
            predictionImg, groundTruthImg,
...
def evaluatePair(
    ...
    predictionNp = predictionImg
    groundTruthNp = groundTruthImg

    # load ground truth instances, if needed
    if args.evalInstLevelScore and instanceImg is not None:
        instanceNp = instanceImg

    # Check for equal image sizes
    if (predictionNp.shape[0] != groundTruthNp.shape[0]):
        printError("Image widths of " + predictionImgName + " and " + groundTruthImgName + " are not equal.")
    if (predictionNp.shape[1] != groundTruthNp.shape[1]):
        printError("Image heights of " + predictionImgName + " and " + groundTruthImgName + " are not equal.")
    ...
    if (CSUPPORT):
        ...
        for (groundTruthImgPixel, predictionImgPixel) in izip(
                np.nditer(groundTruthNp), np.nditer(predictionNp)):
            if (not groundTruthImgPixel in args.evalLabels):
                printError("Unknown label with id {:}".format(groundTruthImgPixel))

            confMatrix[int(groundTruthImgPixel)][int(predictionImgPixel)] += 1
    ...
    if args.evalPixelAccuracy:
        ...
        perImageStats[predictionImgName] = {}
        perImageStats[predictionImgName]["nbNotIgnoredPixels"] = np.count_nonzero(notIgnoredPixels)
        perImageStats[predictionImgName]["nbCorrectPixels"]    = np.count_nonzero(erroneousPixels)
