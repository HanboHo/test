...
sys.path.append( os.path.normpath( os.path.join( os.path.dirname( __file__ ) , '..' , 'evaluation' ) ) )
...
def getGtInstances(groundTruthImgList, groundTruthImgNameList, args):
    if not args.quiet:
        print("Creating ground truth instances from png files.")
    gtInstances = instances2dict(groundTruthImgList, groundTruthImgNameList,
                                 not args.quiet)
...
def matchGtWithPreds(predictionList, groundTruthList, groundTruthNameList,
                     gtInstances, args):
    matches = {}
    if not args.quiet:
    ...
    count = 0
    for (pred, gt, name) in zip(predictionList, groundTruthList,
                                groundTruthNameList):
        # key for dicts
        dictKey = name
        ...
        for label in args.instLabels:
            predInstances[name2label[label].id] = []
        pred_cls = pred // 1000
        pred_cls_unique = np.unique(pred_cls)
        pred_cls_unique = np.delete(pred_cls_unique, [0])
        individual_class_masks = []
        individual_instance_masks = []
        for cls in pred_cls_unique:
            cls = int(cls)
            zero_img = np.zeros(pred.shape)
            zero_img[pred_cls == cls] = 1
            individual_class_masks.append(
                {'name': str(cls),
                 'labelID': cls,
                 'mask': pred * zero_img}
            )
        for single_cls in individual_class_masks:
            pred_ins = single_cls['mask'] % 1000
            pred_ins_unique = np.unique(pred_ins)
            for ins in pred_ins_unique:
                ins = int(ins)
                zero_img = np.zeros(pred.shape)
                zero_img[pred_ins == ins] = 1
                zero_img = zero_img * single_cls['mask'].astype(bool)
                if np.count_nonzero(zero_img * single_cls['mask']) == 0:
                    continue
                individual_instance_masks.append(
                    {'name': single_cls['name'] + '_' + str(ins),
                     'labelID': single_cls['labelID'],
                     'mask': zero_img}
                )

        (curGtInstances, curPredInstances) = assignGt2Preds(
            curGtInstancesOrig, gt, individual_instance_masks, args)
...
def assignGt2Preds(gtInstancesOrig, gtImage, predInfo, args):
    ...
    for predImageFile in predInfo:
        ...
        predInstance = {}
        predInstance["imgName"]          = predImageFile["name"]
        ...
        for (oI,overlapTh) in enumerate(overlaps):
            for (lI,labelName) in enumerate(args.instLabels):
                ...
                if y_true.size != y_score.size:
                    printError("Sizes of `y_true` and `y_score` are not the "
                               "same...")

                # TODO: it may happen that all the pred are outside of gt.
                # TODO: we just make the ap 0 in this case instead of 'nan'
...
def evaluateImgLists(predictionImgNameList, groundTruthImgNameList,
    ...
    gtInstances = getGtInstances(groundTruthImgList, groundTruthImgNameList,
                                 args)
    # match predictions and ground truth
    matches = matchGtWithPreds(predictionImgList, groundTruthImgList,
                               groundTruthImgNameList, gtInstances,
                               args)
